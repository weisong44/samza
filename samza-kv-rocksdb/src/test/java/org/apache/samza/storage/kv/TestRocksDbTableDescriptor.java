...
public class TestRocksDbTableDescriptor {
  ...
  public void testNoKeySerde() {
    new RocksDbTableFactory().getTableDescriptor("1")
        .validate();
  }

  @Test(expected = SamzaException.class)
  public void testNoValueSerde() {
    new RocksDbTableFactory().getTableDescriptor("1")
  ...
  public void testTableSpec() {
    TableSpec tableSpec = new RocksDbTableFactory().getTableDescriptor("1")
        .withKeySerde(new IntegerSerde())
        .withValueSerde(new StringSerde("UTF-8"))
        .withBlockSize(1)
        .withCacheSize(2)
    ...
    Assert.assertEquals(IntegerSerde.class.getName(), tableSpec.getConfig().get(RocksDbTableDescriptor.KEY_SERDE));
    Assert.assertEquals(StringSerde.class.getName(), tableSpec.getConfig().get(RocksDbTableDescriptor.VALUE_SERDE));
